        ...
        <CCardBody>
            ...
            <div class="row">
                ...
                <b-col md="4" xl="4" sm="4">
                    ...
                    <b-button style='margin:2px' class="btn-sm" type="button" @click="inActiveAll()" variant="warning">Inactive</b-button>
                </b-col>
                <b-col md="4" xl="4" sm="6" class="text-right">
                   <span>Total Active: <span class="badge-badge-success">{{ total_active }}</span></span>
                   <span class="ml-3">Total Inactive: <span class="badge-badge-success">{{ total_inactive }}</span></span>
                </b-col>
            </div>
                ...
                <div class="overflow-auto">
                    <b-table thead-class="bg-light text-dark" emptyText="Data Not Found" show-empty bordered hover :items="itemList" :fields="fields">
                        ...
                        <template v-slot:cell(action)="data">
                            <b-button v-if="data.item.status == 2" title="Active" class="btn btn-success mr-2 mt-1 btn-sm" @click="changeStatus(data.item)"> <i class="ri-check-line"></i></b-button>
                            <b-button v-else title="Block" class="mr-2 mt-1 btn btn-danger btn-sm" @click="changeStatus(data.item)"><i class="ri-close-line"></i></b-button>
                            <b-button class="mr-2 mt-1 btn btn-success btn-sm" v-b-modal.modal-1 @click="edit(data.item)"><i class="ri-ball-pen-fill m-0"></i></b-button>
                        </template>
                    </b-table>
                </div>
            </b-overlay>
...
export default {
    ...
    methods: {
        ...
        loadData () {
            ...
            RestApi.getData(baseUrl, 'api/package/list', params).then(response => {
                if (response.success) {
                    ...
                    this.total_active = response.total_active
                    this.total_inactive = response.total_inactive
                }
        ...
        inActiveAll () {
            this.$swal({
                title: 'Are you sure to inactive message ?',
                showCancelButton: true,
                confirmButtonText: 'Yes',
                cancelButtonText: 'No',
                focusConfirm: false
        ...
        inactiveallData () {
            ...
            this.$store.dispatch('mutedLoad', { loading: true })
            RestApi.postData(baseUrl, `api/package/inactive-all`, { data: itemList }).then(response => {
                if (response.success) {
                    this.$store.dispatch('mutedLoad', { listReload: true })
                    iziToast.success({
                        title: 'Success',
