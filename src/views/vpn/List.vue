    ...
    <CCol col>
      <CCard>
        <CCardHeader>
            <div class="row">
                <div class="col-md-6">
                    <CIcon name="cil-justify-center"/><strong> Vpn List</strong>
                </div>
                <div class="col-md-6">
                ...
                <div class="overflow-auto">
                    <b-table thead-class="bg-light text-dark" emptyText="Data Not Found" show-empty bordered hover :items="itemList" :fields="fields">
                        ...
                        <template v-slot:cell(action)="data">
                            ...
                            <b-button title="Delete" class="ml-2 btn btn-danger btn-sm" @click="deleteItem(data.item)"><i class="ri-delete-bin-line"></i></b-button>
...
export default {
    ...
    computed: {
        ...
        fields () {
            const labels = [
                ...
                { label: 'Country Name', class: 'text-left' },
                { label: 'Country Code', class: 'text-center' },
    ...
    methods: {
        ...
        deleteData (item) {
            RestApi.deleteData(baseUrl, `api/vpns/delete/${item.id}`).then(response => {
                if (response.success) {
        ...
        toggleStatus (item, statusId) {
            RestApi.putData(baseUrl, `api/vpns/status/${item.id}`, { status: statusId }).then(response => {
                if (response.success) {
                    this.$store.dispatch('mutedLoad', { listReload: true })
                    iziToast.success({
                        title: 'Success',
        ...
        loadData () {
            ...
            RestApi.getData(baseUrl, 'api/vpns/list', params).then(response => {
                if (response.success) {
