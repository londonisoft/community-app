...
<div>
    ...
    <CCol>
      <CCard>
        <CCardHeader>
            <div class="row">
                <div class="col-md-6">
                    <strong>Exclusive Member</strong>
                </div>
            </div>
        </CCardHeader>
        <CCardBody>
                ...
                <div class="overflow-auto mt-2">
                    <b-table thead-class="bg-light text-dark" emptyText="Data Not Found" small show-empty bordered hover :items="itemList" :fields="fields">
                        ...
                        <template v-slot:cell(action)="data">
                            <b-button title="Delete" class="ml-2 btn btn-danger btn-sm" @click="changeStatus(data.item)"><i class="ri-close-line"></i></b-button>
                        </template>
                    </b-table>
                </div>
            </b-overlay>
  ...
  <b-modal id="modal-1"
    ...
    title="Add Exclusive" hide-footer>
    <div>
		<addEx :id='editId'/>
  </div>
  </b-modal>
...
export default {
    ...
    methods: {
        ...
        toggleStatus (item) {
            RestApi.deleteData(baseUrl, `api/user-signup/user-type/${item.id}`).then(response => {
                if (response.success) {
        ...
        loadData () {
            const params = Object.assign({}, this.search, { page: this.pagination.currentPage, per_page: this.pagination.perPage, member_type: 1 })
