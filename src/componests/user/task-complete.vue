            ...
            <b-overlay :show='loading'>
                <div class="overflow-auto">
                    <b-table thead-class="bg-light text-dark" emptyText="Data Not Found" small show-empty bordered hover :items="itemList" :fields="fields">
                        ...
                        <template v-slot:cell(work_name)="data">
                            <span>{{ data.item.work_name ? data.item.work_name : data.item.package.name }}</span>
                        </template>
...
export default {
    ...
    methods: {
        ...
        loadData () {
            ...
            RestApi.getData(baseUrl, 'api/user-signup/task-list-com', params).then(response => {
                if (response.success) {
